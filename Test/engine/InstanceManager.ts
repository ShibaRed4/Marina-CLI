    ...
    switch (instanceType) {
      case InstanceType.Part:
        return {
          ...
          eventEmitter: enhanceEventEmitterInstance(new EventEmitter()),
          collide: function (
            entity: Instance,
            overlaps: { xOverlap: number; yOverlap: number },
          ) {
            // Create a very simple object that will be easier to convert
            const simpleOverlap = {
              x: overlaps.xOverlap,
              y: overlaps.yOverlap
            };
                        
            // Try using separate parameters instead of an object
            this.eventEmitter.emit("Collided", entity, simpleOverlap.x, simpleOverlap.y);
          },
